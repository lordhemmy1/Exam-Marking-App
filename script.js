...
function addNewEssayGroup(existing) {
  ...
  const imgIn = div.querySelector('.db-essay-img'),
        preview = div.querySelector('.file-preview');
  imgIn.addEventListener('change', e=>{
    const f = e.target.files[0];
    if(!f) return;
    const reader = new FileReader();
    reader.onload = ev => {
      preview.innerHTML = `<img src="${ev.target.result}" alt="${f.name}" class="essay-preview-img">`;
    };
...
function saveStudentData() {
  ...
  let groups=document.querySelectorAll('.essay-group'),
      essays=[];
  for(let g of groups){
    ...
    essays.push({ qNo:q, answer:a, image:imgFile });
  }
...
function renderStudentDBTable() {
  ...
  c.innerHTML = `
    <table id="student-db-table">
      ...
      <tbody>
        ${studentDB.map((s,i)=>{
          let es = s.essayAnswers.map(e=> e.image?`[${e.image}]`:e.answer ).join('; ');
          return `<tr>
...
function loadStudentForMarking() {
  ...
  const student = studentDB
    .find(s=> s.name===name && s.class===cls && s.arm===arm);
  if(!student){ alert('Student not found'); return; }

  // 3. Populate objective by commas
  ...
  // Populate essay section with 4 tools (including Erase)
...
function renderObjectiveMarkingDetails(answers) {
  ...
  let html = `<h4>Objective Details</h4>
    <table><thead><tr><th>Q</th><th>Correct</th><th>Student</th><th>Remark</th></tr></thead><tbody>`;
  for(let i=1;i<=50;i++){
    const stud = answers[i-1]||'';
    const corr = answerData.objectives[i]||'';
    const remark = (corr.toLowerCase()===stud.toLowerCase())?'✔':'✖';
    html += `<tr><td>${i}</td><td>${corr}</td><td>${stud}</td><td>${remark}</td></tr>`;
  }
  html += '</tbody></table>';
  det.innerHTML = html;
}
...
function finalizeMarking() {
  ...
  for(let i=1;i<=50;i++){
    ...
    if(inp && inp.value.trim()
       && answerData.objectives[i]===inp.value.trim()) objScore++;
  }
  // essay score
  let essayScore=0;
  ...
  studentData.push({ name, cls, arm, objScore, essayScore, total: objScore+essayScore });
  persistScores();
  updateScoreTable();
}
...
function initAll() {
  ...
  document.getElementById('objective-marking-form').innerHTML=''; // will fill on upload
